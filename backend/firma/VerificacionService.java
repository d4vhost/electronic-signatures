...
public class VerificacionService {
        ...
        try {
            ...
            try (PDDocument document = PDDocument.load(new ByteArrayInputStream(pdfBytes))) {
                ...
                for (PDSignature sig : signatures) {
                    ...
                    try {
                        // Extraer solo los bytes de la firma (pequeños, no causan OOM)
                        byte[] signatureContent = sig.getContents(new ByteArrayInputStream(pdfBytes));

                        // Crear un CMSTypedData que lee directamente de pdfBytes usando byteRange
                        // Esto evita duplicar el contenido del PDF en memoria
                        final int[] byteRange = sig.getByteRange();
                        org.bouncycastle.cms.CMSTypedData signedContentData = new org.bouncycastle.cms.CMSTypedData() {
                            @Override
                            ...
                            public void write(java.io.OutputStream out) throws java.io.IOException, org.bouncycastle.cms.CMSException {
                                out.write(pdfBytes, byteRange[0], byteRange[1]);
                                out.write(pdfBytes, byteRange[2], byteRange[3]);
                            }
                            @Override
                            public org.bouncycastle.asn1.ASN1ObjectIdentifier getContentType() {
                                return new org.bouncycastle.asn1.ASN1ObjectIdentifier("1.2.840.113549.1.7.1");
                            }
                        };

                        // Parsear la firma CMS/PKCS7 pasándole el contenido firmado para la verificación
                        CMSSignedData cmsSignedData = new CMSSignedData(signedContentData, signatureContent);
                        ...
                        for (SignerInformation signerInfo : signerInfoStore.getSigners()) {
                            ...
                            // Extraer Sellado de Tiempo
                            String selladoTiempoStr = "No";
                            try {
                                org.bouncycastle.asn1.cms.AttributeTable unsignedAttrs = signerInfo.getUnsignedAttributes();
                                if (unsignedAttrs != null) {
                                    org.bouncycastle.asn1.cms.Attribute tsAttr = unsignedAttrs.get(org.bouncycastle.asn1.pkcs.PKCSObjectIdentifiers.id_aa_signatureTimeStampToken);
                                    if (tsAttr != null) {
                                        org.bouncycastle.asn1.ASN1Encodable dobj = tsAttr.getAttrValues().getObjectAt(0);
                                        org.bouncycastle.cms.CMSSignedData tsCms = new org.bouncycastle.cms.CMSSignedData(dobj.toASN1Primitive().getEncoded());
                                        org.bouncycastle.tsp.TimeStampToken tsToken = new org.bouncycastle.tsp.TimeStampToken(tsCms);
                                        
                                        java.util.Date tsDate = tsToken.getTimeStampInfo().getGenTime();
                                        SimpleDateFormat tsSdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                                        tsSdf.setTimeZone(TimeZone.getTimeZone("America/Guayaquil"));
                                        
                                        String tsaIssuer = tsToken.getSID().getIssuer().toString();
                                        selladoTiempoStr = tsSdf.format(tsDate) + " hora de Ecuador Sello de tiempo emitido por " + extractOrganization(tsaIssuer);
                                    }
                                }
                            } catch (Exception e) {
                                LOGGER.warning("Error leyendo sello de tiempo: " + e.getMessage());
                            }
                            ...
                            // LOGGER.info("Subject DN completo: " + subjectDN); // Oculto por seguridad
                            ...
                            LOGGER.info("Cedula y CN extraidos correctamente (datos ocultos por seguridad).");
                            if (!telefono.isEmpty()) {
                            ...
                            firmante.put("selladoTiempo", selladoTiempoStr);
                            ...
                            LOGGER.info("Firma procesada | Entidad: " + entidadCertificadora +
    ...
    public Map<String, Object> validarCertificadoP12(byte[] p12Bytes, String password) throws Exception {
        ...
        result.put("subjectDN", subjectDN);
        result.put("issuerDN", issuerDN);
        
        // Formatear fechas al huso horario de Ecuador
        SimpleDateFormat sdfEc = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        sdfEc.setTimeZone(TimeZone.getTimeZone("America/Guayaquil"));
        result.put("validFrom", sdfEc.format(cert.getNotBefore()) + " - hora de Ecuador");
        result.put("validTo", sdfEc.format(cert.getNotAfter()) + " - hora de Ecuador");
        
        // Extraer campos individuales del SubjectDN usando BouncyCastle
        // (LOGGER.info de SubjectDN removido por privacidad de datos en producción)
        
        org.bouncycastle.asn1.x500.X500Name x500 = new org.bouncycastle.asn1.x500.X500Name(
            cert.getSubjectX500Principal().getName(javax.security.auth.x500.X500Principal.RFC2253));
        
        // Cédula: OID 2.5.4.5 (serialNumber)
        String cedula = getX500Field(x500, org.bouncycastle.asn1.x500.style.BCStyle.SERIALNUMBER);
        if (cedula.isEmpty()) cedula = extractField(subjectDN, "SERIALNUMBER");
        if (cedula.isEmpty()) cedula = extractField(subjectDN, "OID.2.5.4.5");
        
        // EXPLORAR EXTENSIONES PARA ENCONTRAR LA CÉDULA
        if (cedula.isEmpty()) {
            try {
                Set<String> nonCritSet = cert.getNonCriticalExtensionOIDs();
                if (nonCritSet != null) {
                    ...
                    if (cedula.isEmpty()) {
                        for (String oid : nonCritSet) {
                            ...
                            if (extVal != null) {
                                ...
                                if (m.find()) {
                                    String val = m.group(1);
                                    if (!val.startsWith("202") && !val.startsWith("09") && !val.startsWith("08")) { 
                                        cedula = val;
                                        break;
                                    }
                                }
        ...
        // Fallback final: buscar regex en el subject
        if (cedula.isEmpty()) {
            java.util.regex.Matcher m = java.util.regex.Pattern.compile("\\b(\\d{10})\\b").matcher(subjectDN);
            if (m.find()) cedula = m.group(1);
        }
        result.put("cedula", cedula);
        
        // Nombres: OID 2.5.4.42 (givenName)
        String givenName = getX500Field(x500, org.bouncycastle.asn1.x500.style.BCStyle.GIVENNAME);
        if (givenName.isEmpty()) givenName = extractField(subjectDN, "GIVENNAME");
        if (givenName.isEmpty()) givenName = extractField(subjectDN, "OID.2.5.4.42");
        
        // Apellidos: OID 2.5.4.4 (surname)
        String surname = getX500Field(x500, org.bouncycastle.asn1.x500.style.BCStyle.SURNAME);
        if (surname.isEmpty()) surname = extractField(subjectDN, "SURNAME");
        if (surname.isEmpty()) surname = extractField(subjectDN, "OID.2.5.4.4");
        
        // Fallback desde el CN si no vinieron en campos separados
        if (givenName.isEmpty() && surname.isEmpty()) {
            String cn = extractField(subjectDN, "CN");
            if (!cn.isEmpty()) {
                cn = cn.replaceAll("\\b\\d{10}\\b", "").trim();
                String[] parts = cn.split("\\s+");
                if (parts.length >= 4) {
                    givenName = parts[2] + " " + parts[3]; // A veces es Apellidos Nombres
        ...
        result.put("nombres", givenName);
        result.put("apellidos", surname);
        
        // (LOGGER.info de extracción de datos removido por privacidad en producción)
        
        // Extraer emisor legible
        String emisorOrg = extractField(issuerDN, "O");
        String emisorCN = extractField(issuerDN, "CN");
        String emisorDesc = emisorOrg.isEmpty() ? emisorCN : emisorOrg;
        if (!emisorCN.isEmpty() && !emisorOrg.isEmpty() && !emisorCN.equals(emisorOrg)) {
            emisorDesc = emisorOrg + " - " + emisorCN;
        }
        result.put("emisor", emisorDesc);
        ...
        result.put("expirado", expirado ? "SI" : "NO");
        
        // Verificar revocación (básico - sin OCSP por ahora)
        result.put("revocado", "NO");
    ...
    private String getX500Field(org.bouncycastle.asn1.x500.X500Name x500Name, org.bouncycastle.asn1.ASN1ObjectIdentifier oid) {
        try {
            org.bouncycastle.asn1.x500.RDN[] rdns = x500Name.getRDNs(oid);
            if (rdns != null && rdns.length > 0) {
                return org.bouncycastle.asn1.x500.style.IETFUtils.valueToString(rdns[0].getFirst().getValue());
            }
        } catch (Exception e) {
            LOGGER.warning("Error extrayendo campo OID " + oid + ": " + e.getMessage());
        }
        return "";
    }
